...
public:
	...
	friend CORE_API Quat2 operator+(const Quat2& q0, const Quat2& q1);
	friend CORE_API Quat2 operator-(const Quat2& q0, const Quat2& q1);

	friend CORE_API Matrix2& operator*=(Matrix2& m, const Quat2& q);
	friend CORE_API Quat2& operator*=(Quat2& q, const Matrix2& m_);
	friend CORE_API Matrix2 operator*(const Matrix2& m, const Quat2& q);
	friend CORE_API Matrix2 operator*(const Quat2& q, const Matrix2& m_);

	friend CORE_API Vector2& operator*=(Vector2& v, const Quat2& q);
	friend CORE_API Vector2 operator*(const Vector2& v, const Quat2& q);

	friend CORE_API Quat2 operator*(const Quat2& q, f32 val);
	friend CORE_API Quat2 operator*(f32 val, const Quat2& q);
	friend CORE_API Quat2 operator/(const Quat2& q, f32 val);
	friend CORE_API Quat2 operator/(f32 val, const Quat2& q);
